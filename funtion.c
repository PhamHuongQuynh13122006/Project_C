...
/*void saveToFile(struct Student *students, int n, const char *filename) {
    ...
    fwrite(&n, sizeof(int), 1, file); // Save the number of students
    fwrite(students, sizeof(struct Student), n, file); // Save student array
    fclose(file);
...
void loadFromFile(struct Student *students, int *n, const char *filename) {
    ...
    fread(n, sizeof(int), 1, file); // Load the number of students
    fread(students, sizeof(struct Student), *n, file); // Load student array
    fclose(file);
...
void sort_Student_i_or_d(struct Student students[100],int n){
	if (n <= 0) {
        printf("Not have enough students.\n");
        return;
    }
	struct Student temp;
	int check = 0;
